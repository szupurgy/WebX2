                                ...
                                return (
                                    <div key={index} className='hover:scale-95 w-72 transition flex flex-col p-5 gap-5 rounded-2xl md:ml-8 md:pb-10 drop sm:w-1/2 md:size-80 md:gap-1 size-11/12 shadow-2xl justify-center items-center bg-gradient-to-t from-stone-500'>
                                        <div className={`w-full ${termek.akcios ? "" : "hidden"} `}>
                                            <AiOutlinePercentage title='Akciós' className='size-10 hover:animate-pulse flex justify-end items-end' />
                                        </div>
                                        <div>
                        ...
                        termekek && termekek.map((termek, index) => {
                            if ((index+1) == termekek.length) {
                                return (
                                    <div key={index} className='hover:scale-95 w-72 transition flex flex-col p-5 gap-5 rounded-2xl md:ml-8 md:pb-10 drop sm:w-1/2 md:size-80 md:gap-1 size-11/12 shadow-2xl justify-center items-center bg-gradient-to-t from-stone-500'>
                                        <div className={`w-full ${termek.akcios ? "" : "hidden"} `}>
                                            <AiOutlinePercentage title='Akciós' className='size-10 hover:animate-pulse ' />
                                        </div>
                                        <div>
                                            <img src={`http://localhost:8000/uploads/${termek.termekkepek[0].kep}`} className='size-60 md:size-40 hover:scale-110 transition drop-shadow-2xl' />
                                        </div>
